    ...
    <div class="card-body">
        <form action="{{ route("admin.events.store") }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="form-group {{ $errors->has('description') ? 'has-error' : '' }}">
                ...
                <input type="text" id="description" name="description" class="form-control" value="{{ old('description', isset($event) ? $event->description : '') }}" required>
            ...
            <div class="form-group {{ $errors->has('salle') ? 'has-error' : '' }}">
                <label for="salle">{{ trans('cruds.event.fields.salle') }}*</label>
                <select id="salle" name="salle" class="form-control" required>
                    <option value="Salle 1" {{ old('salle', isset($event) && $event->salle === 'Salle 1' ? 'selected' : '') }}>Salle 1</option>
                    <option value="Salle 2" {{ old('salle', isset($event) && $event->salle === 'Salle 2' ? 'selected' : '') }}>Salle 2</option>
                    <option value="Salle 3" {{ old('salle', isset($event) && $event->salle === 'Salle 3' ? 'selected' : '') }}>Salle 3</option>
                </select>
                @if($errors->has('salle'))
                    <em class="invalid-feedback">
                        {{ $errors->first('salle') }}
                ...
                @endif
                <p class="helper-block">
                    {{ trans('cruds.event.fields.salle_helper') }}
                </p>
            </div>
